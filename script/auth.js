...
export const saveUserData = (userId, name, email) => {
    set(ref(database, 'users/' + userId), {
        username: name,
        email,
        createdAt: new Date().toISOString() // 저장 데이터에 생성 시간 추가 (수정된 부분)
...
// 로그인 로직 (app.js에서 호출하도록 변경)
export const loginUser = async (email, password) => {
    try {
        await signInWithEmailAndPassword(auth, email, password);
        console.log('로그인 성공');
        window.location.replace('dashboard.html'); // 로그인 성공 시 대시보드로 이동 (수정된 부분)
    } catch (err) {
...
// 회원가입 로직 (app.js에서 호출하도록 변경)
export const registerUser = async (email, password) => {
    try {
        await createUserWithEmailAndPassword(auth, email, password);
        const userId = auth.currentUser ? auth.currentUser.uid : null;
        if (userId) {
            saveUserData(userId, email, ""); // 회원가입 후 데이터 저장 (수정된 부분)
            window.location.replace('dashboard.html'); // 회원가입 후 대시보드로 이동 (수정된 부분)
        } else {
...
export const googleLogin = async () => {
    try {
        ...
        if (userId) {
            saveUserData(userId, user.displayName, user.email); // Google 로그인 후 데이터 저장 (수정된 부분)
            window.location.replace('dashboard.html'); // Google 로그인 성공 시 대시보드로 이동 (수정된 부분)
        } else {
            console.error('유저 정보가 없습니다.');
        }
    } catch (err) {
